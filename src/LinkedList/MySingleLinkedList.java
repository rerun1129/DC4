...
public class MySingleLinkedList<T> { //단방향 노드의 연결리스트
    ...
    public Node<T> head;  //주소값만 가짐(next) 데이터 필드 정보는 Node 클래스의 data가 item을 입력받아서 가지고 있다.
    public int size;
    ...
    public void addFirst(T item) { //목록의 맨 앞에 끼워넣기(head 만들기랑 똑같다)
    ...
    public void addAfter(Node<T> before, T item) {   //끼워넣을 주소의 앞주소와 데이터 필드 정보
    ...
    public void add(int index, T item) {             //add 메서드들을 만든건 지금까지의 빌드업이었다.
        if (index < 0 || index > size) {           //인덱스 값의 예외사항은 이걸로 처리해주고(일단은 return으로만 처리한다.)
            return;         }                        //여기서 등호가 사라진 이유는 다른 기능들은 index번지를 찾거나 삭제할수는 없다.
    ...
    public T removeFirst() {            //만약에 헤드가 null이면 지울게 없기 때문에 굳이 무슨 작업을 할 필요가 없다.
    ...
    public T remove(int index) {             //이것도 빌드업 한거다 이제 각 메서드를 만들어둔거를 적절하게 가져와서 써먹자.
    ...
    public T remove(T item) {                //이건 데이터 필드의 값을 찾아서 노드를 삭제하는 것이다. 따라서 데이터를 입력받아야한다.
        ...
        if(p==null)                             //이 경우는 while문의 두가지 반복조건 중에 전자만 false인 경우이다.
            return null;                        //후자가 계속 false인 경우는 필드를 못찾은거고 후자까지 조건이 안가는 경우는 head가 null인 경우이다.
        if(q==null)                             //이 경우는 while문의 첫 반복시에 전자는 true 후자가 false인 경우인데 p.next가 null인 경우이다.
            return removeFirst();               //p.next가 null이면 q는 움직일수가 없기 때문에 q도 계속 null상태인것이다.
        else                                    //이 경우는 후자가 false인 상태에서 q가 null이 아닌 상태이다. 이러면 q.next가 p를 지정해서
            return removeAfter(q);              //q.next.next를 q.next에 써버리면 p 주소는 사라지고 p.data는 리턴된다.
    }
    ...
    public Node<T> getNode(int index) {          // 노드 자체를 가져와야 하니까 Node<T>를 타입으로 써야한다.
        if (index < 0 || index >= size)
            return null;
